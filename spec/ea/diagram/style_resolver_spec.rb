...
RSpec.describe Ea::Diagram::StyleResolver do
  let(:resolver) { described_class.new }

  describe "#initialize" do
    it "creates configuration instance" do
      expect(resolver.configuration).to be_a(Ea::Diagram::Configuration)
    end

    it "creates style parser instance" do
      expect(resolver.style_parser).to be_a(Ea::Diagram::StyleParser)
    end

    it "accepts custom config path" do
      custom_resolver = described_class.new("custom/path/config.yml")
      expect(custom_resolver.configuration).to be_a(Ea::Diagram::Configuration)
    end
  end
  ...
  describe "#resolve_connector_style" do
    context "with generalization connector" do
      let(:connector) { Lutaml::Uml::Generalization.new }
    ...
    context "with association connector" do
      let(:connector) do
        Struct.new(:class, :name).new(Struct.new(:name).new("Lutaml::Uml::Association"),
               member_end: [])
      end
  ...
  describe "private methods" do
    describe "#parse_diagram_object_style" do
      it "parses BCol (fill color)", :aggregate_failures do
        ...
        result = resolver.parse_diagram_object_style(style_string)
      ...
      it "parses BFol (bold font)", :aggregate_failures do
        ...
        bold_result = resolver.parse_diagram_object_style(bold_style)
        normal_result = resolver.parse_diagram_object_style(normal_style)
      ...
      it "parses IFol (italic font)", :aggregate_failures do
        ...
        italic_result = resolver.parse_diagram_object_style(italic_style)
        normal_result = resolver.parse_diagram_object_style(normal_style)
      ...
      it "handles nil style string" do
        result = resolver.parse_diagram_object_style(nil)

        expect(result).to eq({})
      end

      it "handles empty style string" do
        result = resolver.parse_diagram_object_style("")
    ...
    describe "#parse_diagram_link_style" do
      it "parses LCol (line color)", :aggregate_failures do
        style_string = "LCol=255;"
        result = resolver.parse_diagram_link_style(style_string)
      ...
      it "parses LStyle for dashed line" do
        dash_style = "LStyle=1;"
        result = resolver.parse_diagram_link_style(dash_style)
      ...
      it "parses LStyle for dotted line" do
        dot_style = "LStyle=2;"
        result = resolver.parse_diagram_link_style(dot_style)

        expect(result[:stroke_dasharray]).to eq("2,2")
      end

      it "handles nil style string" do
        result = resolver.parse_diagram_link_style(nil)
      ...
      it "handles solid line style (LStyle=0)" do
        ...
        result = resolver.parse_diagram_link_style(solid_style)
    ...
    describe "#determine_connector_type" do
      it "returns 'generalization' for Generalization class" do
        connector = Lutaml::Uml::Generalization.new
        type = resolver.determine_connector_type(connector)
      ...
      it "returns 'dependency' for Dependency class" do
        connector = Lutaml::Uml::Dependency.new
        type = resolver.determine_connector_type(connector)
      ...
      it "returns 'realization' for Realization class" do
        connector = Lutaml::Uml::Realization.new
        type = resolver.determine_connector_type(connector)
      ...
      it "defaults to 'association' for unknown types" do
        connector = Struct.new(:dummy).new
        type = resolver.determine_connector_type(connector)
      ...
      it "returns 'association' for nil connector" do
        type = resolver.determine_connector_type(nil)
    ...
    describe "#determine_association_type" do
      it "returns 'aggregation' for aggregation type" do
        ...
        type = resolver.determine_association_type(connector)
