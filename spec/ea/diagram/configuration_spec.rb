...
RSpec.describe Ea::Diagram::Configuration do
  ...
  describe "private methods" do
    describe "#matches_package?" do
      it "matches exact package names" do
        result = config.matches_package?("CityGML::Core", "CityGML::Core")
        expect(result).to be true
      end

      it "matches wildcard patterns" do
        result = config.matches_package?("CityGML::Core", "CityGML::*")
        expect(result).to be true
      end

      it "does not match unrelated packages" do
        result = config.matches_package?("Other::Package", "CityGML::*")
        expect(result).to be false
      end

      it "handles nil package names" do
        result = config.matches_package?(nil, "CityGML::*")
        expect(result).to be false
      end

      it "handles nil patterns" do
        result = config.matches_package?("CityGML::Core", nil)
        expect(result).to be false
      end

      it "handles complex wildcard patterns" do
        result = config.matches_package?("CityGML::Core::Feature",
                             "CityGML::*")
        expect(result).to be true
      end
    end
    ...
    describe "#dig_hash" do
      it "navigates nested hashes with dot notation" do
        hash = { "a" => { "b" => { "c" => "value" } } }
        result = config.dig_hash(hash, "a.b.c")
        expect(result).to eq("value")
      end

      it "returns nil for non-existent paths" do
        hash = { "a" => { "b" => "value" } }
        result = config.dig_hash(hash, "a.x.y")
        expect(result).to be_nil
      end

      it "returns nil for nil path" do
        hash = { "a" => "value" }
        result = config.dig_hash(hash, nil)
        expect(result).to be_nil
      end

      it "returns nil for empty path" do
        hash = { "a" => "value" }
        result = config.dig_hash(hash, "")
        expect(result).to be_nil
      end
    end
    ...
    describe "#deep_merge" do
      it "merges nested hashes", :aggregate_failures do
        hash1 = { "a" => { "b" => 1, "c" => 2 } }
        hash2 = { "a" => { "b" => 3, "d" => 4 } }
        result = config.deep_merge(hash1, hash2)
