...
RSpec.describe "EA Diagram SVG Accuracy" do
  ...
  let(:repository) do
    if File.exist?(lur_path)
      Lutaml::UmlRepository::Repository.from_file(lur_path)
    else
  ...
  let(:reference_dir) { File.expand_path("../../../../lutaml-uml/examples/xmi/Images", __dir__) }
    ...
    describe "diagram: #{diagram_info[:name]}" do
      ...
      context "with EA reference SVG" do
        ...
        let(:generated_svg) do
          extractor = Ea::Diagram::Extractor.new
