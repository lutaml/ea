...
RSpec.describe Ea::Transformations::TransformationEngine do
  # Mock parser for testing
  class MockParser < Ea::Transformations::Parsers::BaseParser
  ...
  class FailingMockParser < Ea::Transformations::Parsers::BaseParser
    def format_name
      "Failing Mock Format"
    end
  ...
  let(:mock_config) do
    config = Ea::Transformations::Configuration.new
    config.version = "1.0"
    config.parsers = [
      Ea::Transformations::Configuration::ParserConfig.new.tap do |p|
        p.format = "mock"
        p.extension = ".mock"
    ...
    config.transformation_options = Ea::Transformations::Configuration::TransformationOptions.new
    config.format_detection = Ea::Transformations::Configuration::FormatDetection.new
    config.error_handling = Ea::Transformations::Configuration::ErrorHandling.new
    config
  end
  ...
  describe "#initialize" do
    it "initializes with default configuration when none provided" do
      ...
      expect(engine.configuration).to be_a(Ea::Transformations::Configuration)
    end
    ...
    it "initializes format registry" do
      expect(engine.format_registry).to be_a(Ea::Transformations::FormatRegistry)
    end
  ...
  describe "#parse" do
    ...
    context "with unsupported file format" do
      ...
      it "raises UnsupportedFormatError" do
        ...
        end.to raise_error(Ea::Transformations::UnsupportedFormatError)
      end
    end
