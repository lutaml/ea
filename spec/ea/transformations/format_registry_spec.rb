...
RSpec.describe Ea::Transformations::FormatRegistry do
  # Mock parser classes for testing
  class MockParser1 < Ea::Transformations::Parsers::BaseParser
    def format_name
      "Mock Format 1"
    end
  ...
  class MockParser2 < Ea::Transformations::Parsers::BaseParser
    def format_name
      "Mock Format 2"
    end
  ...
  class HighPriorityParser < Ea::Transformations::Parsers::BaseParser
    def format_name
  ...
  class LowPriorityParser < Ea::Transformations::Parsers::BaseParser
    def format_name
      "Low Priority Parser"
    end
  ...
  describe "#auto_register_from_parser" do
    ...
    it "validates parser class is a BaseParser subclass" do
      expect do
        registry.auto_register_from_parser(String)
      end.to raise_error(ArgumentError, /inherit from/)
    end

    it "rejects parser with empty extensions" do
      empty_parser = Class.new(Ea::Transformations::Parsers::BaseParser) do
        def format_name = "Empty"
        def supported_extensions = []
        def parse_internal(_) = Lutaml::Uml::Document.new
      end
      expect do
        registry.auto_register_from_parser(empty_parser)
