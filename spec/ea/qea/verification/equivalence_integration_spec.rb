...
RSpec.describe "XMI/QEA Equivalence Integration" do
  ...
  before(:all) do # rubocop:disable RSpec/InstanceVariable
    @verifier = Ea::Qea::Verification::DocumentVerifier.new
  ...
  describe "normalizer" do
    let(:normalizer) { Ea::Qea::Verification::DocumentNormalizer.new }
    let(:xmi_path) { File.join(xmi_dir, "test.xmi") }
    let(:xmi_doc) { Ea::Xmi::Parser.parse(xmi_path) }
  ...
  describe "structure matcher" do
    let(:matcher) { Ea::Qea::Verification::StructureMatcher.new }
    let(:xmi_path) { File.join(xmi_dir, "test.xmi") }
    let(:qea_path) { File.join(qea_dir, "test.qea") }
    let(:xmi_doc) { Ea::Xmi::Parser.parse(xmi_path) }
    let(:qea_doc) { Ea::Qea.parse(qea_path) }
  ...
  describe "element comparator" do
    let(:comparator) { Ea::Qea::Verification::ElementComparator.new }
  ...
  describe "comparison result" do
    let(:result) { Ea::Qea::Verification::ComparisonResult.new }
