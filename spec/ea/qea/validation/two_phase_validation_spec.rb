...
RSpec.describe "Two-Phase Validation System" do
  ...
  let(:database) { Ea::Qea.load_database(qea_file) }
  let(:document) do
    Ea::Qea::Factory::EaToUmlFactory.new(database).create_document
  end
  let(:engine) do
    Ea::Qea::Validation::ValidationEngine.new(document, database: database)
  end

  describe "Validation Engine Initialization" do
    it "creates validation engine with document and database",
       :aggregate_failures do
      expect(engine).to be_a(Ea::Qea::Validation::ValidationEngine)
    ...
    it "sets up default validators in registry", :aggregate_failures do
      ...
      # document_structure lives in lutaml-uml; registration is conditional
      # on it being loadable (namespace compatibility)
  ...
  describe "Phase 1: QEA Database Validation" do
    it "validates database referential integrity", :aggregate_failures do
      result = engine.validate_qea_database(engine.build_context())
  ...
  describe "Phase 2: UML Tree Validation" do
    it "validates UML document structure", :aggregate_failures do
      result = engine.validate_uml_tree(engine.build_context())
  ...
  describe "Validator Separation" do
    it "Phase 1 validators work with database models", :aggregate_failures do
      ...
      expect(db_packages.first.class.name).to eq("Ea::Qea::Models::EaPackage")
    end
