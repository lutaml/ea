...
RSpec.describe "QEA Full Parsing Integration", :integration do
  ...
  describe "Ea::Qea.parse" do
    ...
    context "with custom options" do
      it "accepts custom document name" do
        custom_doc = Ea::Qea.parse(qea_path, document_name: "My Model")
        expect(custom_doc.name).to eq("My Model")
      end

      it "can skip validation" do
        expect do
          Ea::Qea.parse(qea_path, validate: false)
        end.not_to raise_error
      end

      it "can exclude diagrams" do
        expect do
          Ea::Qea.parse(qea_path, include_diagrams: false)
        end.not_to raise_error
      end
    end
  end
  ...
  describe "Integration with UmlRepository" do
    before { skip "UmlRepository not available" unless defined?(Lutaml::UmlRepository) }
  ...
  describe "Performance characteristics" do
    ...
    it "produces document with expected element counts", :aggregate_failures do
      database = Ea::Qea.load_database(qea_path)
  ...
  describe "Error handling" do
    it "raises error for non-existent file" do
      expect do
        Ea::Qea.parse("/non/existent/file.qea")
    ...
    it "handles empty database gracefully", :aggregate_failures do
      ...
      Tempfile.create(["empty", ".qea"]) do |f|
        ...
        Ea::Qea::Services::DatabaseLoader::MODEL_CLASSES.each_key do |table|
          db.execute("CREATE TABLE #{table} (id INTEGER PRIMARY KEY)")
        end
        db.close

        doc = Ea::Qea.parse(f.path)
